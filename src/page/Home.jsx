...
import SkeletonCardVideo from "../components/SkeletonCardVideo";
    ...
    const fetch = async () => {
      try {
        const datas = await axios.get(
          `https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&maxResults=24&regionCode=US&key=${process.env.REACT_APP_YOUTUBE_KEY_1}`
        );
      ...
      const profilePrictures = await Promise.all(
        data.items.map(async (element) => {
          const pp = await axios.get(
            `https://www.googleapis.com/youtube/v3/channels?part=snippet&fields=items%2Fsnippet%2Fthumbnails%2Fdefault&id=${element.snippet.channelId}&key=${process.env.REACT_APP_YOUTUBE_KEY_1}`
          );
  ...
  const nextPage = async () => {
    ...
    try {
      const dataNextPage = await axios.get(
        `https://youtube.googleapis.com/youtube/v3/videos?part=snippet&part=contentDetails&part=statistics&chart=mostPopular&maxResults=24&pageToken=${data.nextPageToken}&regionCode=US&key=${process.env.REACT_APP_YOUTUBE_KEY_1}`
      );
  ...
  const prevPage = async () => {
    ...
    try {
      const dataprevPagePage = await axios.get(
        `https://youtube.googleapis.com/youtube/v3/videos?part=snippet&part=contentDetails&part=statistics&chart=mostPopular&maxResults=24&pageToken=${data.prevPageToken}&regionCode=US&key=${process.env.REACT_APP_YOUTUBE_KEY_1}`
      );
  ...
  return (
    <>
      <div className="flex justify-center">
        <div className="sm:m-[3vw] mt-[5vw] grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 w-[25rem] sm:w-full">
          {loading ? (
            <SkeletonCardVideo count={24} />
          ) : (
